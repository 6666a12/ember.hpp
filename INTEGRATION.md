...
- 构建期 SPIR-V：`-DEMBER_BUILD_SPIRV=ON`（默认）且找到 glslangValidator 时，`ember_spirv` 目标把 `shaders/` 全部编译到 `build/generated/spirv/`（`--target-env vulkan1.0`），供未来 Vulkan 后端直接加载，无运行时编译器依赖
